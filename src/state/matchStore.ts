      ...
      undoLastEvent: () =>
        set((state) => {
          ...
          if (bowlerId && countsAsBall && game.lastBowlerPerOver) {
            ...
            const eventBallNumber = lastEvent.ballNumber;
            if (!eventBallNumber) {
              console.log(
                "⚠️ Missing eventBallNumber — skipping bowler restore",
              );
            } else {
              const eventOverIndex = Math.floor(
                (eventBallNumber - 1) / LEGAL_BALLS,
              );

              const previousBowlerId =
                gameStore.currentGame?.lastBowlerPerOver?.[eventOverIndex];

              console.log("🏏 UNDO RESTORE CHECK (FINAL)", {
                eventBallNumber,
                eventOverIndex,
                currentBowlerId: gameStore.currentGame?.currentBowlerId,
                previousBowlerId,
                lastBowlerPerOver: gameStore.currentGame?.lastBowlerPerOver,
              });

              if (previousBowlerId && !game.explicitBowlerSelection) {
                console.log("🏏 RESTORING PREVIOUS BOWLER", {
                  from: gameStore.currentGame?.currentBowlerId,
                  to: previousBowlerId,
                });

                gameStore.setCurrentBowler(previousBowlerId);
              } else {
                console.log("⚠️ No restore condition met", {
                  hasPreviousBowler: !!previousBowlerId,
                  explicitBowlerSelection: game.explicitBowlerSelection,
                });
              }
            }
          }
